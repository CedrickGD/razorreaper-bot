...
3. **Variables** tab — add:
   ...
   | `PRESETS_CHANNEL_ID` | *(channel id)* | Optional — the presets forum (every active licence) the verify panel links. Found by name once otherwise (`preset`, text or forum) and remembered in `ids.json`; never created, and left off the panel until it exists. |
