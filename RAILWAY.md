...
3. **Variables** tab — add:
   ...
   | `EARLY_CHANNEL_ID` | *(channel id)* | Optional — the early-commits feed (active licences; the GitHub push webhook posts there) the verify panel lists as a Discord perk. Found by name once otherwise (`early`), never created. |
